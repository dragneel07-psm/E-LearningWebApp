...
from academic.models import Exam, ExamSeating, Student, Assessment, Section, Subject, AcademicClass
...
def ensure_test_data(db_alias):
    ...
    if not section:
        ...
        if not academic_class:
            academic_class = AcademicClass.objects.using(db_alias).create(name="Grade 10", level=10)
        section = Section.objects.using(db_alias).create(name="A", academic_class=academic_class)
        print(f"✅ Created Section: {section}")

    # Ensure section has a student
    student = Student.objects.using(db_alias).filter(section=section, user__is_active=True).first()
    if not student:
    ...
    assessment = Assessment.objects.using(db_alias).filter(section=section, type='exam').first()
    if not assessment:
        subject = Subject.objects.using(db_alias).filter(academic_class=section.academic_class).first()
        if not subject:
    ...
    try:
        ...
        if seating_list.exists():
            ...
            for s in seating_list:
                print(f"   - {s.student.user.username}: Seat {s.seat_number}, Hall Ticket: {s.hall_ticket_number}")
        else:
