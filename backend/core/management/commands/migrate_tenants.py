...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--tenant_id', type=str, help='Specific tenant ID to migrate')
    ...
    def handle(self, *args, **options):
        ...
        for tenant in tenants:
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f"Successfully migrated {tenant.db_alias}"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Failed to migrate {tenant.db_alias}: {str(e)}"))
