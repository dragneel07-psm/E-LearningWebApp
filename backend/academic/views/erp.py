...
class SchoolERPOverviewView(APIView):
    ...
    def get(self, request):
        ...
        admissions_status_counts = {
            ...
            for row in admissions_qs.values('status').annotate(total=Count('enquiry_id'))
        }
        ...
        try:
            ...
            if tenant is not None:
                ...
                pending_fees_qs = StudentFee.objects.filter(status__in=['pending', 'partial', 'overdue'])
            ...
            total_due = pending_fees_qs.aggregate(total=Sum('amount_due')).get('total') or 0
            total_paid = pending_fees_qs.aggregate(total=Sum('amount_paid')).get('total') or 0
        ...
        return Response(
            {
                ...
                'admissions': {
                    ...
                    'application_started': int(admissions_status_counts.get('application_started', 0)),
