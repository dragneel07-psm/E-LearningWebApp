...
class AttendanceViewSet(TenantScopedQuerysetMixin, viewsets.ModelViewSet):
    ...
    def initial(self, request, *args, **kwargs):
        ...
        if self.action in {"create", "update", "partial_update", "destroy"} and not self._can_edit():
    ...
    def _teacher_scope_queryset(self, queryset):
        ...
        taught_subject_filter = (
            models.Q(subject__teacher=teacher_profile)
            | models.Q(subject__additional_teachers=teacher_profile)
        )
        class_filter = models.Q(student__academic_class_id__in=class_ids) if class_ids else models.Q(pk__in=[])
    ...
    def get_queryset(self):
        ...
        if role == "student":
            ...
            queryset = queryset.filter(student=student_profile) if student_profile else queryset.none()
        elif role == "parent":
            parent_profile = Parent.objects.prefetch_related("students").filter(user=self.request.user).first()
    ...
    def _ensure_teacher_can_modify(self, student, subject):
        ...
        manages_subject = subject.teacher_id == teacher_profile.teacher_id or subject.additional_teachers.filter(
            teacher_id=teacher_profile.teacher_id
        ).exists()
        manages_class = teacher_profile.assigned_classes.filter(id=student.academic_class_id).exists() if student.academic_class_id else False
        if not (manages_subject or manages_class):
            raise PermissionDenied("You can only mark attendance for your assigned subjects/classes.")
