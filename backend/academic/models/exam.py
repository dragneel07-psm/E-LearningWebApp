...
class Exam(models.Model):
    ...
    assessment = models.OneToOneField(Assessment, on_delete=models.CASCADE, related_name='exam_details', limit_choices_to={'type': 'exam'})
    ...
    def generate_hall_ticket_number(self, student):
        ...
        class_name = student.academic_class.name.replace(' ', '') if student.academic_class else "NA"
        section_name = student.section.name if student.section else "NA"
        student_part = str(student.student_id)[:6] if student.student_id else str(student.id)[:6]
        
        return f"{self.hall_ticket_prefix}-{class_name}-{section_name}-{student_part}".upper()

class ExamSeating(models.Model):
    seating_id = models.UUIDField(primary_key=True, default=uuid_lib.uuid4, editable=False)
