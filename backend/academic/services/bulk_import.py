...
from academic.models import Student, AcademicClass, Section
    ...
    def process_file(self, file_obj):
        ...
        try:
            ...
            if file_obj.name.endswith('.csv'):
                ...
                if csv_reader.fieldnames:
                    fieldnames = [f.lower().strip().replace(' ', '_') for f in csv_reader.fieldnames]
                    csv_reader.fieldnames = fieldnames
                else:
                    return {'success': False, 'error': "CSV file appears to be empty or missing headers"}
            ...
            # 2. Handle Excel using pandas (Fallback if broken)
                ...
                try:
                    ...
                    rows = df.to_dict('records')
                     ...
                     return {'success': False, 'error': f"Excel import requires properly installed pandas/openpyxl. Please use CSV instead. Error: {ie}"}
        ...
        for index, row in enumerate(rows):
            ...
            try:
                ...
                if not email:
                    results['failed'] += 1
                    results['errors'].append({'row': row_idx, 'error': "Email is required"})
                    continue

                with transaction.atomic():
                    # Lookup Class
                    academic_class = AcademicClass.objects.filter(name__iexact=class_name).first()
                    ...
                    section = None
                    if section_name:
                        section = Section.objects.filter(name__iexact=section_name, academic_class=academic_class).first()
                        if not section:
                             raise Exception(f"Section '{section_name}' not found for class '{class_name}'")
