...
class ReportingService:
    ...
    def generate_student_report(self, student_id: str, save: bool = True, is_automated: bool = False, using: str = 'default') -> Dict:
        """
        Generate a comprehensive AI-driven performance report for a student.
        """
        try:
            student = Student.objects.using(using).select_related('user', 'academic_class', 'section').get(student_id=student_id)
            
            # 1. Gather Performance Data
            results = Result.objects.using(using).filter(student=student).select_related('assessment', 'assessment__subject').order_by('-submitted_at')
            avg_score = results.aggregate(Avg('score'))['score__avg'] or 0
            
            # 2. Topic Mastery
            subjects = Subject.objects.using(using).filter(academic_class=student.academic_class)
            mastery_data = []
            for sub in subjects:
            ...
            att_records = Attendance.objects.using(using).filter(student=student, date__gte=thirty_days_ago)
            att_rate = 0
    ...
    def generate_class_summary(self, class_id: int, using: str = 'default') -> Dict:
        ...
        try:
            ...
            students = Student.objects.using(using).filter(academic_class=academic_class)
            ...
            summary_context = {
                ...
                "top_performing": students.annotate(avg=Avg('results__score')).order_by('-avg')[:3]
            }
