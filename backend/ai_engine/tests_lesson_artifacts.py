...
class LessonArtifactEndpointTests(FastTenantTestCase):
    ...
    def setUp(self):
        with tenant_context(self.tenant):
            ...
            chapter = Chapter.objects.create(subject=subject, title="Motion", order=1, is_published=True)
    ...
    def test_summarize_endpoint_returns_expected_payload_and_caches(self, mock_call_model):
        ...
        response = self.client.post(f"/api/ai/lessons/{self.lesson.id}/summarize/?lang=en", format="json")
    ...
    def test_invalid_lang_returns_400(self):
        response = self.client.post(f"/api/ai/lessons/{self.lesson.id}/exam_notes/?lang=fr", format="json")
