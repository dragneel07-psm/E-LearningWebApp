            ...
            try:
                ...
                for student in students:
                    try:
                        ...
                        self.stdout.write(self.style.ERROR(f"Error for student {student.pk}: {str(e)}"))
                
                self.stdout.write(self.style.SUCCESS(f"Processed {count} students for {tenant.name}"))
            
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Failed to process tenant {tenant.name}: {str(e)}"))
            
            finally:
                # Cleanup
                if hasattr(_thread_locals, 'tenant'): del _thread_locals.tenant
                if hasattr(_thread_locals, 'db_alias'): del _thread_locals.db_alias
