...
class HostelBlockViewSet(TenantFilterMixin, viewsets.ModelViewSet):
    ...
    def dashboard(self, request):
        ...
        total_capacity = sum(
            sum(r.capacity for r in b.rooms.all())
            for b in blocks
        )
...
class HostelRoomViewSet(TenantFilterMixin, viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated]
    queryset = HostelRoom.objects.select_related('block').prefetch_related('allotments').all()
...
class HostelAllotmentViewSet(TenantFilterMixin, viewsets.ModelViewSet):
    ...
    def _create_hostel_fee(self, allotment, tenant):
        ...
        try:
            ...
            due_date = allotment.check_in_date.replace(day=1) if hasattr(allotment.check_in_date, 'replace') else datetime.date.today().replace(day=1)
