...
with schema_context("demo"):
    from academic.models import AcademicClass, Teacher, Subject, Section, Assessment
    from users.models import UserAccount
    
    cls_10, _ = AcademicClass.objects.get_or_create(name="Class 10", defaults={"grade_level": 10})
    ...
    # Create Subjects
    math_sub, _ = Subject.objects.get_or_create(name="Mathematics", academic_class=cls_10, defaults={"code": "MATH10", "teacher": math_teacher, "credits": 4.0})
    Subject.objects.filter(id=math_sub.id).update(teacher=math_teacher) # ensure teacher
    
    sci_sub, _ = Subject.objects.get_or_create(name="Science", academic_class=cls_10, defaults={"code": "SCI10", "teacher": sci_teacher, "credits": 4.0})
