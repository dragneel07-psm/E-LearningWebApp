...
def verify_learning_path():
    ...
    resp = requests.post(f"{BASE_URL}/learning-paths/generate/", headers=headers, json={})
    ...
    if active_path.get('nodes'):
        ...
        resp = requests.post(f"{BASE_URL}/learning-nodes/{node_id}/complete/", headers=headers)
        if resp.status_code == 200:
            result = resp.json()
            print(f"✅ Node marked complete. Next node unlocked: {result['next_node_unlocked']}")
        else:
