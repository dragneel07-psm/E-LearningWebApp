...
def handle_ai_response(conversation_id, user_message_content, user):
    ...
    try:
        ...
        if conversation.type == 'direct':
            ...
            ai_participant = conversation.participants.filter(
        ...
        elif conversation.type == 'group' and user_message_content.strip().lower().startswith('@ai'):
            is_triggered = True
            user_message_content = user_message_content.lower().replace('@ai', '', 1).strip()
        ...
        for msg in reversed(history):
            role = "assistant" if msg.sender.email == "ai-assistant@apeesys.com" else "user"
